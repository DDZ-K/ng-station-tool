...
**v1.6.1** — XML 与产品 DMC 正反面扫码互认：固定第 14～16 位 `000`/`001` 视为同一件（或关系，写死不开放配置）；可叠加 `_S1` 等站位后缀。

**v1.6.0** — 产线服务料号白名单：配置中心可维护多条料号；产品 DMC（一级文件夹名）包含任一料号才改名入 A/待 NG，否则整夹忽略。

**v1.5.2** — 待 NG / 待判断队列旁增加「清空队列」按钮（仅清内存排队，不删磁盘文件）。
...
```text
图片源目录\产品DMC文件夹
          ↓ 产品DMC 须包含任一维护料号（空列表=不过滤）
          ↓ 整夹静默后改名
A 目录（待 NG 队列）  ← 拷贝成功即入队，显示完整图片名 + 产品DMC
          ↓ XML identifier 匹配产品文件夹（精确 / 000·001 正反面 / _S1 后缀）
B 目录\年\月\日（待判断队列）
...
- 在「配置中心 → 服务料号」中添加/删除多条料号（一般约 10 位）。
- 匹配规则：`产品DMC.Contains(料号)`，多料号任意命中即可。
- 列表为空时不过滤（兼容旧配置）；产线使用请填入需要服务的料号。
- 未命中：不拷贝到 A、不入待 NG，日志记一次跳过。
